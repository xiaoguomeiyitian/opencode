...
it.effect("Alibaba Messages normalizes completed tool calls while preserving raw and truncated finishes", () =>
  Effect.gen(function* () {
    for (const item of [
      { tool: true, reason: "end_turn", expected: "tool-calls" },
      { tool: true, reason: "max_tokens", expected: "length" },
      { tool: false, reason: "end_turn", expected: "stop" },
    ]) {
              ...
              input.respond(
                sseEvents(
                  ...
                  ...(item.tool
                    ...
                    : []),
                  { type: "message_delta", delta: { stop_reason: item.reason }, usage: { output_tokens: 10 } },
                  { type: "message_stop" },
                ),
                { headers: { "content-type": "text/event-stream" } },
              ),
      ...
      expect(response.finishReason).toEqual({ normalized: item.expected, raw: item.reason })
      expect(response.toolCalls).toHaveLength(item.tool ? 1 : 0)
    }
  }),
)
